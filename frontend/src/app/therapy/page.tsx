...
const ACTIVITY_BANK = [
  { name: '?°ì±…', emoji: '?š¶' }, { name: '?…ì„œ', emoji: '?“š' },
  { name: '?Œì•…ê°ìƒ', emoji: '?µ' }, { name: '?”ë¦¬', emoji: '?³' },
  { name: '?¤íŠ¸?ˆì¹­', emoji: '?§˜' }, { name: '?¼ê¸°?°ê¸°', emoji: '?ï¸' },
  { name: '?í™”ê°ìƒ', emoji: '?¬' }, { name: 'ì¹œêµ¬?°ë½', emoji: '?“±' },
  { name: 'ëª…ìƒ', emoji: '?Œ¿' }, { name: 'ê·¸ë¦¼ê·¸ë¦¬ê¸?, emoji: '?¨' },
];
...
export default function TherapyPage() {
  ...
  const playAudio = (base64: string) => {
    ...
    audio.play().catch(e => console.warn("[TTS] ?¬ìƒ ?¤íŒ¨", e));
  };

  const startSession = () => {
    setSessionStarted(true);
    setActiveTab('therapy');
    sendMessage("?ˆë…•?˜ì„¸?? ?ë‹´???œì‘?˜ê³  ?¶ì–´??");
  };
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("?œë²„ ?‘ë‹µ ?¤ë¥˜");
      ...
      console.error("ì±„íŒ… ?¤ë¥˜:", error);
      setMessages([...newMessages, { role: 'assistant', content: "ì£„ì†¡?´ìš”, ? ìƒ?˜ê³¼ ?°ê²°??? ì‹œ ?Šê²¼?´ìš”. ?„ë˜ 'ì¢…ë£Œ' ë²„íŠ¼???„ë¥´ê³??¤ì‹œ ?œë„??ì£¼ì„¸??" }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="flex-1 overflow-y-auto px-4 py-8 space-y-6 bg-white pb-32">
        <div className="text-center space-y-1">
          <h2 className="text-2xl font-black text-gray-900 tracking-tighter">?œë™ ê¸°ë¡ì§€</h2>
          ...
          <table className="w-full border-collapse text-center text-[12px]">
            <thead>
              <tr className="bg-primary/30 text-gray-900 font-bold border-b-2 border-primary/20">
                <th className="py-4 px-2 border-r border-primary/10 w-20">? ì§œ/?œê°„</th>
                <th className="py-4 px-4 border-r border-primary/10">?œë™ ë°??ë‹´ ?´ìš©</th>
                <th className="py-4 px-2">ê¸°ë¶„</th>
              </tr>
            </thead>
            <tbody className="text-gray-800 font-medium">
              {sortedRecords.length > 0 ? (
                ...
                <tr>
                  <td colSpan={3} className="py-20 text-gray-300 font-bold">ê¸°ë¡???°ì´?°ê? ?†ìŠµ?ˆë‹¤.</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  const ActivityView = () => {
    ...
    // 2. ì£¼ì°¨ë³??¬ë ¥ ?°ì´??    const progStart = getProgramStart(userRecords);
    ...
    // 3. ê°ì‚¬ ?¼ê¸° ?€??    const handleSaveGratitude = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto px-4 py-8 space-y-6 bg-white pb-32">
        <div className="text-center space-y-1">
          <h2 className="text-2xl font-black text-gray-900 tracking-tighter">?¤ëŠ˜???ê¸°ê´€ë¦?/h2>
          <div className="w-12 h-1 bg-primary mx-auto rounded-full" />
        </div>

        {/* 1. ë¬´ë“œ ê²Œì´ì§€ ì¹´ë“œ */}
        <div className="bg-white rounded-[40px] p-8 border-2 border-primary/20 shadow-sm relative overflow-hidden">
          <div className="flex flex-col items-center">
            ...
            <p className="text-sm font-bold text-gray-400 text-center leading-relaxed">
              {avgScore >= 7 ? "?˜Š ?„ë°˜?ìœ¼ë¡??„ì£¼ ?ˆì •?ì¸ ?íƒœ?…ë‹ˆ??" : avgScore >= 4 ? "?˜ ê¸°ë¶„ ?„í™˜???„í•´ ?¤ëŠ˜ ì¶”ì²œ ?œë™???´ë³´?¸ìš”." : "?˜” ë§ˆìŒ?´ìŒê³???ê¹Šì? ?€?”ê? ?„ìš”???œê°„?´ì—??"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-8 border-2 border-primary/20 shadow-sm">
           <div className="flex justify-between items-center mb-6">
             <span className="font-black text-gray-900 text-lg">ê¸°ë¶„ ?¬ë ¥</span>
             <div className="flex items-center gap-4 bg-gray-50 px-3 py-1.5 rounded-2xl">
                ...
                <span className="text-xs font-black text-primary-dark">{ledgerWeek}ì£¼ì°¨</span>
           ...
           <div className="grid grid-cols-7 gap-3">
              {weekDays.map((day, i) => (
                <div key={i} className="flex flex-col items-center gap-2">
                   <span className="text-[10px] font-black text-gray-400">{['??,'??,'??,'ëª?,'ê¸?,'??,'??][i]}</span>
        ...
        {avgScore >= 7 && (
          <div className="bg-secondary/50 rounded-[40px] p-8 border-2 border-primary/30 flex items-center gap-6">
             <div className="w-16 h-16 bg-white rounded-3xl flex items-center justify-center text-3xl shadow-sm border border-primary/10">?ƒ</div>
             <div className="flex-1">
                <p className="text-xs font-bold text-gray-400 mb-1">?˜ë? ?‰ë³µ?˜ê²Œ ë§Œë“  ?œë™</p>
                <h4 className="font-black text-gray-900 text-lg leading-tight tracking-tighter">ê¸°ë¶„??ì¢‹ì•˜????<br/>ì£¼ë¡œ <span className="text-primary-dark underline underline-offset-4">?°ì±…</span>???˜ì…¨?¤ìš”!</h4>
             </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-[40px] p-8 border-2 border-primary/20 shadow-sm space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="font-black text-gray-900 text-lg">?œë™ ?¼ì • ê´€ë¦?/h3>
          ...
          <div className="space-y-3 pt-4">
             {scheduledActivities.length > 0 ? (
               scheduledActivities.map((sa, i) => (
                 <div key={i} className="bg-gray-50 p-4 rounded-3xl flex items-center gap-4 group">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] font-bold text-gray-400">{sa.scheduledTime} Â· {sa.dateKey.split('-')[2]}??/p>
                    </div>
               ...
               <p className="text-gray-300 font-bold text-center py-4">?ë¬¼??ë¹„ë£Œë¥?ì£¼ëŠ” ?¼ì •???†ìŠµ?ˆë‹¤.</p>
             )}
          </div>
        </div>
        ...
        {/* 5. ê°ì‚¬ ?¼ê¸° */}
        <div className="bg-white rounded-[40px] p-8 border-2 border-primary/20 shadow-sm space-y-6">
           <h3 className="font-black text-gray-900 text-lg">?“” ê°ì‚¬ ?¼ê¸°</h3>
           <div className="space-y-4">
              ...
              <textarea 
                ...
                placeholder="?¤ëŠ˜ ê°ì‚¬?ˆë˜ ?°ëœ»???œê°„????ê±¸ìŒ ?ì–´ë³´ì„¸??.." 
              ...
              <div className="flex items-center justify-between pt-2">
                 ...
                 <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black text-primary-dark flex items-center gap-1"><MapPin size={10} /> ?‰í™”ë¡œìš´ ?¼ìƒ</span>
              ...
              {gratitudeEntries.map((e, i) => (
                <div key={i} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative group overflow-hidden">
                   <div className="flex items-center justify-between mb-3">
                      <div className="px-3 py-1 bg-secondary text-primary-dark text-[10px] font-black rounded-lg">{e.date} Â· {e.location}</div>
        ...
        {showSatisfactionModal && (
          <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[48px] p-10 w-full max-w-sm text-center shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-2">?œë™ ?„ë£Œ! ?‰</h3>
                <p className="text-sm font-bold text-gray-400 mb-8">?œë™ ??ê¸°ë¶„?€ ?´ë– ? ê???</p>
                ...
                <div className="flex gap-3">
                   ...
                   <button onClick={handleSaveSatisfaction} className="flex-1 py-4 bg-primary rounded-3xl font-black text-gray-900 shadow-lg">?ìˆ˜ ?€??/button>
                </div>
             </motion.div>
          </div>
        )}
      ...
      {!sessionStarted ? (
        <>
            ...
            {activeTab === 'therapy' && (
              ...
              >
                ...
                <div className="text-center space-y-4 px-8">
                   <h2 className="text-5xl font-black text-gray-900 leading-[1.1] tracking-tighter">ë§ˆìŒ?´ìŒê³?br />?€?”í•˜ê¸?/h2>
                   <p className="text-[22px] text-gray-900/60 font-bold leading-relaxed px-4">?´ë¥´? ì˜ ë§ˆìŒ???°ëœ»?˜ê²Œ<br />?¤ì–´ì£¼ëŠ” ?€?”ê? ì¤€ë¹„ë˜?ˆìŠµ?ˆë‹¤.</p>
                </div>
                <footer className="px-6 py-10 w-full mt-auto">
                   ...
                   >
                     <div className="flex flex-col">
                        ...
                        <span className="text-3xl font-black text-gray-900 tracking-tighter">?µí™” ?œì‘?˜ê¸°</span>
                     </div>
          ...
          <div className="bg-white/95 backdrop-blur-2xl border-t border-gray-100 h-24 flex justify-around items-center px-10 z-[60] fixed bottom-0 max-w-md w-full">
             ...
             <div className="flex flex-col items-center gap-1 cursor-pointer" onClick={() => setActiveTab('therapy')}>
               ...
               <span className={`text-[10px] font-black ${activeTab === 'therapy' ? 'text-gray-900' : 'text-gray-400'}`}>?œë™</span>
             </div>
             ...
             <div className="flex flex-col items-center gap-1 cursor-pointer" onClick={() => setActiveTab('record')}>
               ...
               <span className={`text-[10px] font-black ${activeTab === 'record' ? 'text-gray-900' : 'text-gray-400'}`}>ê¸°ë¡</span>
             </div>
             ...
             <div className="flex flex-col items-center gap-1 cursor-pointer" onClick={() => setActiveTab('activity')}>
               ...
               <span className={`text-[10px] font-black ${activeTab === 'activity' ? 'text-gray-900' : 'text-gray-400'}`}>?ê¸°ê´€ë¦?/span>
             </div>
          </div>
        </>
      ) : (
        ...
        <div className="flex-1 flex flex-col h-full">
          <header className="flex justify-between items-center py-6 px-8 sticky top-0 bg-background/80 backdrop-blur-md z-30">
            ...
            <div className="flex flex-col items-center">
               <span className="text-2xl font-black tracking-tight">ë§ˆìŒ?´ìŒ</span>
