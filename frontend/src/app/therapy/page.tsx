...
export default function TherapyPage() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages([...newMessages, { role: 'assistant', content: "죄송해요, 선생님과 연결이 잠시 끊겼어요. 아래 '종료' 버튼을 누르고 다시 시도해 주세요." }]);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {sortedRecords.length > 0 ? (
                sortedRecords.slice(0, 15).map((row, i) => (
                  <tr key={i} className="border-b border-primary/10 last:border-0 h-16 odd:bg-primary/5">
                    ...
                    <td className="font-black text-primary-dark drop-shadow-sm px-2 text-sm italic">
                ...
                <tr>
                  <td colSpan={3} className="py-20 text-gray-300 font-bold italic">기록된 데이터가 없습니다.</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  const ActivityView = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto px-4 py-8 space-y-6 bg-white pb-32">
        <div className="text-center space-y-1">
          <h2 className="text-2xl font-black text-gray-900 tracking-tighter italic">오늘의 자기관리</h2>
          <div className="w-12 h-1 bg-primary mx-auto rounded-full" />
        </div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-48 h-24 mb-6">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-3">
                <span className="text-4xl font-black text-gray-900 italic tracking-tighter">{avgScore.toFixed(1)}</span>
            ...
            <p className="text-sm font-bold text-gray-400 text-center leading-relaxed">
              {avgScore >= 7 ? "😊 전반적으로 아주 안정적인 상태입니다." : avgScore >= 4 ? "😐 기분 전환을 위해 오늘 추천 활동을 해보세요." : "😔 마음 선생님과 더 깊은 대화가 필요한 시간이에요."}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-8 border-2 border-primary/20 shadow-sm space-y-6">
          ...
          <div className="space-y-3 pt-4">
             {scheduledActivities.length > 0 ? (
                 ...
                 <div key={i} className="bg-gray-50 p-4 rounded-3xl flex items-center gap-4 group">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {sa.satisfaction && <div className="px-2 py-0.5 bg-emerald-100 text-emerald-600 text-[9px] font-black rounded-full italic">{sa.satisfaction} pt</div>}
                      </div>
               ...
               <p className="text-gray-300 font-bold italic text-center py-4">식물에 비료를 주는 일정이 없습니다.</p>
             )}
          </div>
        </div>
           ...
           <div className="space-y-4">
              <div className="flex gap-2">
                 {['nature', 'home', 'work'].map(theme => (
                    <button key={theme} className="w-16 h-12 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-center text-xl hover:bg-primary/10 transition-all italic font-black text-gray-300">#</button>
                 ))}
              ...
              <textarea 
                ...
                placeholder="오늘 감사했던 따뜻한 순간을 한 걸음 적어보세요..." 
              ...
              <div className="flex items-center justify-between pt-2">
                 ...
                 <div className="flex items-center gap-4">
                    ...
                    <button onClick={handleSaveGratitude} className="bg-primary text-gray-900 px-6 py-3 rounded-full font-black text-sm shadow-premium active:scale-95 transition-all">저장</button>
                 </div>
              </div>
           </div>
           ...
           <div className="space-y-4 pt-6">
              {gratitudeEntries.map((e, i) => (
                <div key={i} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative group overflow-hidden">
                   ...
                   <p className="text-sm font-bold text-gray-600 leading-relaxed italic">"{e.text}"</p>
        ...
        {showSatisfactionModal && (
          <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[48px] p-10 w-full max-w-sm text-center shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-2 italic">활동 완료! 🎉</h3>
                ...
                <div className="space-y-6 mb-10">
                   ...
                   <div className="text-6xl font-black text-gray-900 italic tracking-tighter">{satisfactionScore}</div>
                </div>
                <div className="flex gap-3">
                   <button onClick={() => setShowSatisfactionModal(false)} className="flex-1 py-4 font-black text-gray-400">Cancel</button>
                   <button onClick={handleSaveSatisfaction} className="flex-1 py-4 bg-primary rounded-3xl font-black text-gray-900 shadow-lg italic">점수 저장</button>
                </div>
             </motion.div>
          </div>
        )}
      ...
      {!sessionStarted ? (
            ...
            {activeTab === 'therapy' && (
              ...
              >
                ...
                <div className="text-center space-y-4 px-8">
                   <h2 className="text-5xl font-black text-gray-900 leading-[1.1] tracking-tighter italic">마음 선생님과<br />대화하기</h2>
                ...
                <footer className="px-6 py-10 w-full mt-auto">
                   ...
                   >
                     <div className="flex flex-col">
                        ...
                        <span className="text-3xl font-black text-gray-900 tracking-tighter italic">통화 시작하기</span>
                     </div>
        ...
        <div className="flex-1 flex flex-col h-full">
          <header className="flex justify-between items-center py-6 px-8 sticky top-0 bg-background/80 backdrop-blur-md z-30">
            ...
            <div className="flex flex-col items-center">
               <span className="text-2xl font-black italic tracking-tight">마음 선생님</span>
